...
import { Shovel, Grid3X3, Leaf, Fence, Scissors, Package, Plus, Sparkles, PlayCircle } from 'lucide-react';
...
export default function DemoPrijzenPage() {
  ...
  const fetchPricing = async () => {
    try {
      const response = await fetch('/api/demo/pricing?all=true');
      if (response.ok) {
  ...
  const updateItem = async (id: string, updates: Partial<PricingItem>) => {
    setSaving(true);
    try {
      const response = await fetch(`/api/demo/pricing/${id}`, {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json' },
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-slate-100 via-slate-50 to-white flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-slate-600 font-medium">Prijzen laden...</p>
        </div>
      </div>
    );
  }
          ...
          <CardContent className="pt-6">
            <div className="flex gap-2 flex-wrap">
              <Button
                ...
                className={activeCategory === 'all' ? 'bg-purple-700 hover:bg-purple-800' : ''}
              >
              ...
              {categories.map((cat) => {
                ...
                return (
                  <Button
                    ...
                    className={`flex items-center gap-1 ${activeCategory === cat.value ? 'bg-purple-700 hover:bg-purple-800' : ''}`}
                  >
