...
export default function PDFDownloadButton({ quote }: PDFDownloadButtonProps) {
  ...
  useEffect(() => {
    ...
    // Set a timeout - if PDF doesn't load in 10 seconds, show error
  ...
  const generatePDF = async () => {
    ...
    try {
      ...
      // Handle missing leads data gracefully
            ...
            {quote.valid_until && (
              <View style={styles.validity}>
                <Text style={styles.validityText}>
                  {`Deze offerte is geldig tot ${formatDate(quote.valid_until)}. Na deze datum kunnen prijzen afwijken.`}
                </Text>
              </View>
            )}
